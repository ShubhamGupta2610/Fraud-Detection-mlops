...
| 4 — API Service | Complete | 2026-07-01 | FastAPI /score endpoint built async, stateless, with pydantic validation, SHAP, decision engine, DB logging, and in-process feature cache. Model loads once at startup (5ms latency per call vs 500ms+ if loaded per-request). All endpoints tested: 16 new API tests + 43 total. Three real bugs found and fixed during testing: path resolution, lifespan triggering in TestClient, cold-start context requirement for detection. |
